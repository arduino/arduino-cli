...
type filesystem interface {
	...
	fs.Writer
	fs.Remover
	// An identifier for the file system
	Name() string
}
...
func (c *RethinkFS) Write(sketch *Sketch, old *Sketch) error {
	// save on fs
	...
	// set StorageProvider name in the sketch
	sketch.StorageProvider = c.FS.Name()
...
func (c *RethinkFS) Delete(id string, fromFSOnly bool) error {
	sketch, err := c.Read(id)
	if sketch == nil {
		return err
	}

	// TODO (Lorenzo): an FS might be able to delete the containing folder
	// directly; in that case we should just delete the folder without caring to delete each single file
	...
	// Also delete the sketch's base folder
	sketch.Generate()
	err = c.FS.Remove(sketch.BasePath)
	if err != nil && !errors.IsNotFound(err) {
		return errors.Annotatef(err, "remove file %s", sketch.BasePath)
	}
